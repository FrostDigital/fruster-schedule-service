...
class CronRunner {
	...
	synchronize() {
		return this.repo.findAllScheduledAndRunning().then(stagedJobs => {
			...
			if (this._isAnyRunning()) {
				...
				if(matchedIds.length != existingIds.length) {
					let jobsToRemove = existingIds.filter((existingJobId) => {
						return !matchedIds.includes(existingJobId);
					});
					this.jobs = this.jobs.filter(job => !jobsToRemove.includes(job.id));					
					log.debug(`Removed ${jobsToRemove.length} jobs`);
				}
			}
...
class Job {
	...
	constructor(model, repo) {
		...
		// TODO: Find out how configurable timezone needs to be
