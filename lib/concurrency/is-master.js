...
/**
 * Checks if current instance is master. 
 * Inspired by https://github.com/mattpker/node-is-master
 */
class IsMaster {

    constructor(db) {
        this.id = uuid.v4();
        this.collection = db.collection("is-master");  
        this.collection.createIndex({id: 1}, true);      
    }

    start() {
        this.addNode();
        setTimeout(this.ping, ms(conf.masterCheckInterval));
    }
    ...
    ping() {
        ...
        this.collection.findOneAndModify(query, update).then(res => {
